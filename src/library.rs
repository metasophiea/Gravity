...
pub fn read_lines<P>(filename:P) -> std::io::Result<Lines<BufReader<File>>> where P: AsRef<Path> {
...
pub fn get_full_address_of_file(address:&str) -> Result<String, String> {
    match canonicalize(PathBuf::from(&address)) {
        Ok(file) => Ok(file.to_str().unwrap().to_string()),
